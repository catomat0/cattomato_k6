...
> **이 프로젝트에는 미구현.** 본 서버 기준으로 설계되어 있어 적용 시 운영 장애 위험이 있습니다.

VU를 상한 없이 계속 늘리다가 임계값이 깨지는 순간 자동으로 테스트를 중단하는 방식입니다. "서버가 최대 몇 명까지 버티는지"를 역산할 수 있습니다.

현재 stress 테스트와의 차이:

| | Stress | Breakpoint |
|---|---|---|
| VU 상한 | 사전에 지정 (`STRESS_MAX_VU`) | 미지정, 계속 증가 |
| 종료 조건 | 시간 | 임계값 초과 시 자동 중단 (`abortOnFail: true`) |
| 알 수 있는 것 | 정해진 부하를 버티는지 | 몇 명까지 버틸 수 있는지 |

**도입 조건:**
- 운영 서버가 아닌 **전용 스테이징 환경** 필요
- EC2 단일 인스턴스에 DB, Redis, 앱이 함께 올라간 구조에서는 터졌을 때 원인 구분 불가 + 실 서비스 영향

아키텍처가 분리되고 스테이징 환경이 갖춰진 이후 도입을 권장합니다.
